...
private function item_exec_action_cb(item as Ihandle ptr) as long
	...
	if ext->iniciar(gui->opcoes) then
		var errCnt = 0
...
function EfdGUI.buildDlg(efdFrm as IHandle ptr) as IHandle ptr
...
function EfdGUI.build() as boolean
	var dlg = buildDlg( _
		buildFileGrid(FG_EFD, "EFD's", "SPED*.txt", "Arquivos do SPED|SPED*.txt"))
