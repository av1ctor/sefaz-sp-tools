...
function PdfTemplate.parseDoubleArray(value as string, toArr() as double, delim as string) as integer
	if len(value) = 0 then
		return 0
	end if
...
function PdfTemplate.getXmlAttribAsDoubleArray(name_ as zstring ptr, toArr() as double, delim as string) as integer
	...
	return parseDoubleArray(value, toArr(), delim)
end function
...
function PdfTemplate.invertY(inFile as string, outFile as string) as boolean
	...
	dim matrix as FS_MATRIX = (1, 0, 0, 1, 0, 0)
	...
	do while xmlTextReaderRead(reader) = 1 		
		...
		case XML_READER_TYPE_ELEMENT
			...
			do while xmlTextReaderMoveToNextAttribute(reader) = 1
				...
				var value = *cast(const zstring ptr, xmlTextReaderConstValue(reader))
				if isPage then
					if *attrib = "mediabox" then
						dim mediaboxArr() as double
						var arrCnt = parseDoubleArray(value, mediaboxArr())
						if arrCnt = 4 then
							h = mediaboxArr(3)
						end if
					end if
					...
					case "transform"
						dim transfArr() as double
						var arrCnt = parseDoubleArray(value, transfArr())
						if arrCnt = 6 then
							matrix.a = transfArr(0)
							matrix.b = transfArr(1)
							matrix.c = transfArr(2)
							matrix.d = transfArr(3)
							matrix.e = transfArr(4)
							matrix.f = transfArr(5)
						end if
						
					case "y"
						var y = cdbl(value)
						var t = 0 + matrix.d * y + matrix.f
						value = str(h - t)
					end select
				end if
				print #outf, " " + *attrib + "=""" + value + """";
